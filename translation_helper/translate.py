...
# load german cards
data_de = json.load(open('api_export_de.json', encoding = "ISO-8859-1"))
cards_de = data_de["cards"]

# load english cards
data_en = json.load(open('api_export_en.json', encoding = "ISO-8859-1"))
...
renamed_cards = json.load(open('renamed_cards.json', encoding = "ISO-8859-1"))

# load ship translations
ship_translations = json.load(open('ship_translations.json', encoding = "ISO-8859-1"))

# load phrase translations
phrase_translations = json.load(open('phrase_translations.json', encoding = "ISO-8859-1"))
...
for card_en in cards_en:
    ...
    if card_en["available_actions"]:
        ...
        for action in card_en["available_actions"]:
            if actions:
                actions += " %s  "%phrase_translations["or"]
            actions += ("<r>%s</r>" if action["base_action_side_effect"] else "%s")% actions_by_id[action["base_action_id"]]
            card_de["ability_text"] = "<i>"+phrase_translations["adds"]%actions +"</i>"+"%LINEBREAK%" + card_de["ability_text"]
